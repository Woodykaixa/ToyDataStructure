...
namespace cym {
	...
	public:
		...
		template <typename T>
		static void quick_sort_s(T arr[], const size_t size, const bool inverse = false) {
			int max = 0;
			if (size < 2) {
				return;
			}
		...
		template <typename T>
		static void heap_sort(T arr[], const size_t size, const bool inverse = false) {
	...
	private:
		...
		template <typename T>
		static void _wrapper_radix_sort(element_wrapper<T> arr[], const size_t size, const size_t radix, size_t max_bit) {
			...
			for (size_t i = 0; i < max_bit; ++i) {
				...
				while (current != -1) {
					const int queue_index = arr[current].get_nth_bit(i + 1, radix);
	...
	public:
		template <typename T>
		static void radix_sort(T arr[], const size_t size, const size_t radix, const bool inverse = false) {
			size_t max_index = 0;
