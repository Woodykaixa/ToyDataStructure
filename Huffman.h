...
public:
	...
	unsigned int get_weight() const {
		return symbol_weight_;
	}

	huff_node* left() const {
		return left_;
	}

	huff_node* right() const {
		return right_;
	}
...
public:
	...
	symbol(const symbol_type&& type, const char c = '\0', const unsigned int weight = 0) {
		type_ = type;
		ch_ = c;
		weight_ = weight;
		rate_ = 0;
	...
	void append_symbol(huff_node::append_dir&& dir, symbol* s) {
		...
		if (node_->left_ != nullptr && node_->right_ != nullptr) {
			node_->symbol_rate_ = node_->left_->symbol_rate_ + node_->right_->symbol_rate_;
			node_->symbol_weight_ = node_->left_->symbol_weight_ + node_->right_->symbol_weight_;
			node_->symbol_value_ = node_->left_->symbol_value_ + node_->right_->symbol_value_;
...
public:
	...
	void calculate_rate() const {
		for (int i = 0; i < symbol_end_index_; ++i) {
			symbols_->at(i)->set_rate(double(symbols_->at(i)->get_weight()) / total_weight_ * 100);
		}
	}
...
private:
	void sort(const int begin, const int end) const {
		...
		while (lb <= hb) {
			while (symbols_->at(lb)->get_rate() < pivot->get_rate() && lb < hb) {
				lb++;
			}
...
private:
	void recursive_scan_tree(huff_node* node, std::string str, huff_node::append_dir dir) {
...
public:
	...
	std::string encode_string(const char* str) {
		...
		std::string result;
		const char* format = "<error: cannot convert to huffman code from char: %c>";
		char buffer[53];
...
private:
	...
	huff_node* parse_code(std::string& result, huff_node* node, std::string& code) const {
