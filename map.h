...
namespace cym {
    ...
    class map {
      ...
      public:
        explicit map(const size_t size = 16, const double factor = 0.75)
            : _used(0), _factor(factor) {
      ...
      public:
        ...
        void put(K key, V v) {
            ...
            for (pair_t* node = _el[i]; node->next() != nullptr;
                 node = node->next()) {
                node->set_next(p);
            }
        }
        ...
        size_t get_used_count() const { return _used; }
