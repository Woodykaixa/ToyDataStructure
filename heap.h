...
namespace cym {
	...
	public:
		...
		heap(T* elements, const size_t length, const heap_type type) : heap(type) {
	...
	private:
		 ...
		 * 对于最大堆，应该判断 parent < item，对于最小堆则是判断 parent > item。
		 */
		...
		static int parent_for(const int child) {
			return (child - 1) / 2;
		}
	...
	public:
		...
		void insert(T e) {
			int i = _used;
			_used++;
			for (; _move_parent_to_child(_el->at(parent_for(i)), e) && i != 0; i = parent_for(i)) {
	...
	public:
		void delete_top() {
			...
			for (parent_index = 0; parent_index < _used; parent_index = child_index) {
				if (parent_index == 0) {
					child_index = 1;
				} else {
				...
				if (child_index < _used - 1 &&
					_use_right_child(_el->at(child_index), _el->at(child_index + 1))) {
					child_index++;
				}
