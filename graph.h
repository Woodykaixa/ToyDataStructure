...
namespace cym {
    ...
    class graph {
      public:
        class edge {
          public:
            ...
            bool operator==(const edge& rhs) const {
                ...
                if (_directed) {
                    return this->_from == rhs._from && this->_to == rhs._to &&
                ...
                int sub1 =
                    static_cast<int>(this->_to) - static_cast<int>(this->_from);
                int sub2 =
                    static_cast<int>(rhs._to) - static_cast<int>(rhs._from);
                return (sub1 + sub2 == 0) || (sub1 - sub2 == 0);
            }

            bool operator!=(const edge& rhs) const { return !(*this == rhs); }

            unsigned int from() const { return _from; }

            void set_from(unsigned int from) { _from = from; }

            unsigned int to() const { return _to; }

            void set_to(unsigned int to) { _to = to; }
        ...
        struct dist {
            ...
            explicit dist(int len = infinity, int pre = -1, int index = -1)
                : len(len), pre(pre), index(index) {}

            dist(const dist&) = default;

            bool operator<(const dist& rhs) const { return len < rhs.len; }

            bool operator>(const dist& rhs) const { return rhs < *this; }

            bool operator<=(const dist& rhs) const { return !(rhs < *this); }

            bool operator>=(const dist& rhs) const { return !(*this < rhs); }
        };
    ...
    class directed_graph : public graph {
      ...
      public:
        explicit directed_graph(size_t v_count)
            : _v_count(v_count), _e_count(0) {
        ...
        size_t edge_count() const override { return _e_count; }

        size_t vertex_count() const override { return _v_count; }
            ...
            for (int i = 0; i < _v_count; ++i) {
                ...
                for (int u = 0; u < _v_count; ++u) {
                    ...
                    if (dists[u].len == dist::infinity ||
                        dists[u].len > dists[v].len + _connection_matrix->visit(
                                                          {begin, end})) {
                        dists[u].len = dists[v].len +
                                       _connection_matrix->visit({begin, end});
                        dists[u].pre = v;
      ...
      private:
        static void ensure_non_negative_weight(matrix_t& matrix,
                                               const size_t from,
                                               const size_t to) {
            ...
            for (size_t i = 0; i < _v_count; ++i) {
                for (size_t j = 0; j < _v_count; ++j) {
                    for (size_t k = 0; k < _v_count; ++k) {
                        ...
                        const bool ij_is_infinity =
                            path_matrix.visit({i, j}) == dist::infinity;
                        const bool others_are_not_infinity =
                            path_matrix.visit({i, k}) != dist::infinity &&
                            path_matrix.visit({k, j}) != dist::infinity;
                        const int ik_kj_weight_sum = path_matrix.visit({i, k}) +
                                                     path_matrix.visit({k, j});
                        const bool has_shorter_path =
                            path_matrix.visit({i, j}) > ik_kj_weight_sum;

                        if (others_are_not_infinity &&
                            (ij_is_infinity || has_shorter_path)) {
                            path_matrix.visit({i, j}) = ik_kj_weight_sum;
                        }
                    }
                }
            ...
            while (vertex_set.count() != _v_count) {
                ...
                for (size_t i = 0; i < added.size(); i++) {
                    for (size_t j = 0; j < to_add.size(); j++) {
                        int current_weight =
                            _connection_matrix->visit({added[i], to_add[j]});
